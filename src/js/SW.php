...
const PRECACHE_ASSETS = <?php echo json_encode(array_column(array_map(fn($r) => ['Name' => $r['name'] ?? '—', 'Method' => strtoupper($r['method'] ?? '—'), 'URL' => $r['url'] ?? '—', 'Full Link' => $r['link'] ?? '—'], PHRO::routes()), 'URL')); ?>;
